...
# Code node 1: Build Groq Request (Prepares payload for HTTP node)
build_groq_payload_code = """
const items = $input.all().map(i => i.json);

if (!items.length) {
  return [{ json: { groq_payload: null, total_analyzed_tracks: 0 } }];
}
...
return [{ json: { groq_payload: payload, total_analyzed_tracks: items.length } }];
"""

# Code node 2: Validate & Format Report (Parses HTTP node output)
...
def main():
    ...
    # 2. Ensure node "Call Groq API" is an httpRequest node with hardcoded Bearer token header
    call_groq_node = {
        "parameters": {
            "method": "POST",
    ...
    # Check if Call Groq API exists, update or add
    found_groq = False
    for i, node in enumerate(wf['nodes']):
        if node['name'] == 'Call Groq API':
            wf['nodes'][i] = call_groq_node
            found_groq = True
            break
    if not found_groq:
        wf['nodes'].append(call_groq_node)

    # 3. Update connections:
    # Join Weekly Spins -> Build Groq Request -> Call Groq API -> Validate & Format Report
    ...
    print("Updated workflow JSON locally with dedicated Call Groq API HTTP node!")
    ...
    print("Waiting 20 seconds for Groq AI to process all 1,894 tracks...")
    time.sleep(20)
