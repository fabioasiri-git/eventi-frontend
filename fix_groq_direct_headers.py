...
def main():
    ...
    # Clean Call Groq API node to use explicit header parameters without credentials
    for node in wf['nodes']:
        if node['name'] == 'Call Groq API':
            node['parameters'] = {
                ...
                'body': '={{ JSON.stringify($json.groq_body) }}',
                'options': {
                    'response': {
                        'response': {
                            'fullResponse': True,
...
conn.commit()
conn.close()
print('SQLite updated with explicit header authorization on Call Groq API node!')
"""
    ...
    print("Waiting 15 seconds for n8n container to recover...")
    time.sleep(15)
    ...
    if execs:
        ...
        if 'Call Groq API' in run_data:
            ...
            if 'body' in out and 'choices' in out['body']:
                print("Groq AI Response preview:", out['body']['choices'][0]['message']['content'][:300])
