...
def main():
    ...
    # 1. Update Build Groq Request code node
    build_groq_code = r"""
...
try {
  const response = await $helpers.httpRequest({
    ...
    headers: {
      'Authorization': 'Bearer ' + (process.env.GROQ_API_KEY || 'gsk_GROQ_KEY_ENV'),
...
"""
    ...
    for node in wf['nodes']:
        if node['name'] == 'Build Groq Request':
            ...
            # Set to ONLY test email address for test safety
            node['parameters']['toEmail'] = 'fabio.asiri@radiotoscana.it, fabio.asiri@gmail.com'
            node['parameters']['html'] = "={{ $json.html_report }}"

    # Reconnect Build Groq Request directly to Validate & Format Report (bypassing old Call Groq API HTTP node)
...
conn.commit()
conn.close()
print('SQLite updated with perfect Groq AI analysis & test email safety!')
"""
    ...
    print("Waiting 18 seconds for n8n container to recover...")
    time.sleep(18)

    # Trigger report webhook
    print("Triggering test report webhook...")
